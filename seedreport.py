# -*- coding: utf-8 -*-
"""
Created on Thu Oct 20 17:56:50 2022
...
def Update():
      ...
      cursor=con.execute('UPDATE addseed SET Seed_Name = ?,Cost_Range = ?, Description = ?,Photo = ? ''WHERE Seed_ID = ?', (str(seedname1.get()),str(costrange1.get()),str(description1.get()),int(seedid1.get()),blob_data))
      con.commit()

def Delete():
      con = sqlite3.connect('project.db')
      cursor = con.cursor()
      query = "DELETE from addseed where Seed_ID = ?"
...
def Displaydata():
    con = sqlite3.connect('project.db')
    cursor=con.execute("SELECT * FROM addseed")
...
label_0 = Label(top,text="View Seed Report",width=30,font=("bold",30))
label_0.place(x=340,y=20)

label_1 = Label(top,text="Seed ID",width=20,font=("bold",20))
label_1.place(x=10,y=110)

entry_1=Entry(top,textvar=seedid1,width=30)
entry_1.place(x=290,y=120)

label_2 = Label(top,text="Seed Name",width=20,font=("bold",20))
label_2.place(x=10,y=170)

entry_2=Entry(top,textvar=seedname1,width=30)
...
tree = ttk.Treeview(sideViewForm,columns=("Seed_ID","Seed_Name","Cost_Range","Description","Photo"),selectmode="extended", height=17, yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
scrollbary.config(command=tree.yview)
scrollbary.pack(side=RIGHT, fill=Y)
scrollbarx.config(command=tree.xview)
scrollbarx.pack(side=BOTTOM, fill=X)
tree.heading('Seed_ID', text="Seed_ID", anchor=W)
tree.heading('Seed_Name', text="Seed_Name", anchor=W)
