# -*- coding: utf-8 -*-
"""
Created on Thu Oct 20 12:46:04 2022
...
top.title("Add Vegetable")

vegetable_id=IntVar()
vegetable_name=StringVar()
...
def database():
    vegetableid1=vegetable_id.get()
    vegetablename1=vegetable_name.get()
    costrange1=cost_range.get()
    description1=description.get()
    conn=sqlite3.connect('project.db')
    with conn:
        cursor=conn.cursor()
    cursor.execute('INSERT INTO addvegetable (Vegetable_ID,Vegetable_Name,Cost_Range,Description,Photo) VALUES(?,?,?,?,?)',(vegetableid1,vegetablename1,costrange1,description1,blob_data))
    conn.commit()
    messagebox.showinfo("Message","Data inserted Successfully")
    sqlite.connect('cache.db',timeout=20)
def back():
    top.destroy()
    import adminhomepage


label_0 = Label(top,text="ADD VEGETABLES",width=30,font=("bold",30))
label_0.place(x=70,y=20)

label_1 = Label(top,text="Vegetable ID",width=20,font=("bold",20))
label_1.place(x=90,y=110)

entry_1=Entry(top,textvar=vegetable_id,width=30)
entry_1.place(x=380,y=120)

label_2 = Label(top,text="Vegetable Name",width=20,font=("bold",20))
label_2.place(x=90,y=170)

entry_2=Entry(top,textvar=vegetable_name,width=30)
